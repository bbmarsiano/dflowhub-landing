...
export default function OnPremise({ product }: { product: Product }) {
  ...
  return (
    <section id="onpremise" className="py-24" style={{ background: "var(--brand-faint)" }}>
      <div className="max-w-4xl mx-auto px-6 text-center">
        <span className="inline-block text-xs font-semibold uppercase tracking-widest mb-6 px-3 py-1 rounded-full" style={{ color: "var(--brand-color)", background: "var(--brand-dim)" }}>
          {t("badge")}
        </span>
        ...
        <div className="inline-flex flex-col sm:flex-row items-center gap-3 px-8 py-5 rounded-2xl" style={{ background: "white", border: "1px solid var(--border)", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 flex-shrink-0" style={{ color: "var(--brand-color)" }}>
            <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
            <polyline points="22,6 12,13 2,6"/>
          </svg>
          <span className="text-base" style={{ color: "var(--text-muted)" }}>
            {t("contact")}{" "}
            <a href={`mailto:${t("email")}`} className="font-semibold hover:underline" style={{ color: "var(--brand-color)" }}>
              {t("email")}
            </a>
          </span>
        </div>
        <div className="mt-12 grid sm:grid-cols-3 gap-6 text-left">
          {benefits.map(({ icon, title, desc }) => (
            <div key={title} className="bg-white rounded-xl p-5" style={{ border: "1px solid var(--border)" }}>
              <div className="w-9 h-9 rounded-lg flex items-center justify-center mb-3 text-lg" style={{ background: "var(--brand-faint)" }}>{icon}</div>
